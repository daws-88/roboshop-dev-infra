...
resource "aws_lb_target_group" "catalogue" {
  ...
  protocol = "HTTP"
  vpc_id = local.vpc_id
  deregistration_delay = 60 # waiting time to before instance delete
...
resource "aws_launch_template" "catalogue" {
  ...
  vpc_security_group_ids = [local.catalogue_sg_id]
  update_default_version = true # if we run terraform again, new version created with new ami
...
## for multiple tags, additional tags can be specified
dynamic "tag" {
  for_each = merge(
    local.common_tags,
    {
 ...
 timeouts {
    delete = "15m"  # wait up to 15 minutes while deleting the Auto Scaling Group resource.
  }
}
...
### create listiner rule for alb
resource "aws_lb_listener_rule" "catalogue" {
  listener_arn = local.backend_alb_listiner_arn
  priority  = 10 # this rule is checked 10th
